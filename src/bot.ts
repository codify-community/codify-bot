...
async function createBotDatabase() {
    ...
    if (!guild) {
        Log.error(
            `❌ Guild com ID ${env.DISCORD_GUILD_ID} não encontrada no cache.`,
        )
        return
    }
...
client.once(Events.ClientReady, async (c) => {
    ...
    try {
        Log.info(
            `🔄 Registrando a lista de slash commands ao bot ${c.user.tag}...`,
        )
        await rest.put(
        ...
        Log.success(
            `✅ slash commands registrados ao bot ${c.user.tag} com sucesso!`,
        )
    } catch (error) {
