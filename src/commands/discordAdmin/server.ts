        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) =>
                    option
                        .setName('key')
                        .setDescription('Nome da variável')
                        .setRequired(true),
                )
                .addStringOption((option) =>
                    option
                        .setName('value')
                        .setDescription('Valor a ser definido')
                        .setRequired(true),
                ),
        ),
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === 'env') {
            if (
                ...
                return interaction.reply({
                    content:
                        'Você precisa ser moderador para usar esse comando.',
                    flags: MessageFlags.Ephemeral,
                })
            }
            ...
            if (key !== 'initialMoneyValue') {
                return interaction.reply({
                    content: 'Chave de ambiente inválida.',
                    flags: MessageFlags.Ephemeral,
                })
            } else {
                try {
                    ...
                    return interaction.reply({
                        content: 'Erro ao atualizar variável de ambiente.',
                        flags: MessageFlags.Ephemeral,
                    })
                }
            }
